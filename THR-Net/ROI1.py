    ...
    for j in range(filearr.shape[0]):
        ...
        if os.path.exists(videofolder):
            ...
            for i in range(int(fps * (13 + 10 * (count))), int(fps * (23 + 10 * (count)))-2):
                ...
                noseimg = cv2.imread(os.path.join(videofolder, 'forehead_{0}.jpg'.format(i)))
                noseimg = cv2.resize(noseimg, (78, 82))
                avg_nose = np.zeros((int(82 / 4) * int(78 / 4), 3))
                avg_nose_count = 0

                for p in range(int(82 / 4)):
                    for q in range(int(78 / 4)):
...
def models(labelarr,imgarr):
    ...
    pretrainmodel=load_model('/home/som/lab/seed-yzj/newpaper4/laboratory/model/labotary_nose_att.hdf5',compile=False)
    ...
    model_checkpoint = ModelCheckpoint('/home/som/lab/seed-yzj/newpaper4/laboratory/model/labotary_forehead_att.hdf5', monitor='val_loss', verbose=1, save_best_only=True,save_weights_only=False)
...
if __name__ == '__main__':
    ...
    imgnpy='/home/som/lab/seed-yzj/newpaper4/laboratory/datanpy/labotary-forehead-2D.npy'
