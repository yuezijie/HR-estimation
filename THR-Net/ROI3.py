    ...
    for j in range(filearr.shape[0]):
        ...
        if os.path.exists(videofolder):
            ...
            for i in range(int(fps * (13 + 10 * (count))), int(fps * (23 + 10 * (count)))-2):
                ...
                noseimg = cv2.imread(os.path.join(videofolder, 'leftcheek_{0}.jpg'.format(i)))
                noseimg = cv2.resize(noseimg, (49, 94))
                avg_nose = np.zeros((int(94 / 4) * int(49 / 4), 3))
                avg_nose_count = 0

                for p in range(int(94 / 4)):
                    for q in range(int(49/ 4)):
...
def models(labelarr,imgarr):
    ...
    model_checkpoint = ModelCheckpoint('/home/som/lab/seed-yzj/newpaper4/laboratory/model/labotary_left_att.hdf5', monitor='val_loss', verbose=1, save_best_only=True,save_weights_only=False)
...
if __name__ == '__main__':
    ...
    imgnpy='/home/som/lab/seed-yzj/newpaper4/laboratory/datanpy/labotary-left-2D.npy'
