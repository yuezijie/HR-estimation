    ...
    for j in range(filearr.shape[0]):
        ...
        if os.path.exists(videofolder):
            ...
            for i in range(int(fps * (13 + 10 * (count))), int(fps * (23 + 10 * (count)))-2):
                ...
                noseimg = cv2.imread(os.path.join(videofolder, 'nose_{0}.jpg'.format(i)))
                noseimg = cv2.resize(noseimg, (80, 67))
                avg_nose = np.zeros((int(67 / 4) * int(80 / 4), 3))
                avg_nose_count = 0

                for p in range(int(67 / 4)):
                    for q in range(int(80 / 4)):
...
def models(labelarr,imgarr):
    ...
    fc6=Dense(units=labelarr.shape[1])(x)
    ...
    model.compile(optimizer=Adam(lr=1e-4),loss=losses.mean_squared_error, metrics=[metrics.MeanAbsoluteError(),
                                                                metrics.MeanAbsolutePercentageError(),
                                                                metrics.RootMeanSquaredError(),pearson_r])

    model_checkpoint = ModelCheckpoint('/home/som/lab/seed-yzj/newpaper4/laboratory/model/labotary_nose_att_new.hdf5', monitor='val_loss', verbose=1, save_best_only=True,save_weights_only=False)
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=15, mode='auto',factor=0.7,min_lr=1e-6)

    # csv_logger = CSVLogger('training.csv')

    model.fit(x_train, y_train,batch_size=24,epochs=200,verbose=1,callbacks=[model_checkpoint,reduce_lr],
...
if __name__ == '__main__':
    ...
    imgnpy='/home/som/lab/seed-yzj/newpaper4/laboratory/datanpy/labotary-nose-2D.npy'
